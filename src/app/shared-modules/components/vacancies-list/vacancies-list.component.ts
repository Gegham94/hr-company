...
export class VacanciesListComponent extends Unsubscribe implements OnInit, OnDestroy {
  ...
  @Output() selectedVacancy: EventEmitter<SearchableSelectDataInterface> =
    new EventEmitter<SearchableSelectDataInterface>();

  @Output() vacancyList: EventEmitter<SearchableSelectDataInterface[]> =
    new EventEmitter<SearchableSelectDataInterface[]>();

  private searchParams: BehaviorSubject<{ status: string, payedStatus: string }> =
    new BehaviorSubject<{ status: string; payedStatus: string }>({
      status: this.status,
  ...
  public vacancySearchList$: BehaviorSubject<SearchableSelectDataInterface[]> =
    new BehaviorSubject<SearchableSelectDataInterface[]>([this.all]);
  ...
  public selectVacancy(selectedVacancy: SearchableSelectDataInterface): void {
  ...
  private getSelectedPaginationValue(): void {
    ...
    this._vacancyFacade.getVacanciesForSelection(this.searchParams.value)
      ...
      .subscribe((vacancies) => {
        if(this.isAll) {
          this.vacancySearchList$.next(
            JSON.parse(JSON.stringify([all, ...vacancies]))
          );
        } else {
          this.vacancySearchList$.next(
            JSON.parse(JSON.stringify([...vacancies]))
          );
        }
